...
async def remember(req: RememberRequest, _=Depends(verify_token)):
    ''''''
    await handle_risk("L1", '', req.content[:50])
...
async def learn(req: LearnRequest, _=Depends(verify_token)):
    ''''''
    await handle_risk("L1", '', req.topic)
...
async def cleanup_memory(days_old: int = 30, _=Depends(verify_token)):
    ''''''
    await handle_risk("L2", f"{days_old}")
...
async def sync_platform(platform: str, _=Depends(verify_token)):
    ...
    try:
        ...
        if platform == "telegram":
            return MemorySync.push_to_telegram('')
        elif platform == "wechat":
            return MemorySync.push_to_wechat('')
        else:
            return {"ok":True,"message":'',"local":MemorySync.sync_push({"synced":True,"time":__import__("time").time()})}
