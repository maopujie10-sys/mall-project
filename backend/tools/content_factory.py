...
class ContentFactory:
    ...
    async def generate_marketing_copy(cls, campaign: dict) -> str:
        ...
        try:
            from agents.multi_model import ModelRouter
            mr = ModelRouter()
            resp = await mr.chat(messages=[{"role":"user","content":f"为以下营销活动写一条吸引人的推广文案(50-100字),带emoji:\n活动:{campaign.get('name','')}\n优惠:{campaign.get('discount','')}\n目标人群:{campaign.get('audience','')}\n平台:{campaign.get('platform','全平台')}"}], mode="fast")
            return resp.get("content", "")
        except:
            return f"🎉 {campaign.get('name','')}来啦！{campaign.get('discount','')}，限时优惠，速来抢购！#好物推荐"
    ...
    async def generate_social_post(cls, topic: dict) -> str:
        ...
        try:
            from agents.multi_model import ModelRouter
            mr = ModelRouter()
            resp = await mr.chat(messages=[{"role":"user","content":f"写一条社交媒体帖子(100-200字),带3-5个相关hashtag:\n主题:{topic.get('theme','')}\n风格:{topic.get('style','轻松活泼')}\n包含: {topic.get('include','')}"}], mode="fast")
            return resp.get("content", "")
        except:
    ...
    async def translate_content(cls, text: str, target_lang: str = "en") -> str:
        ...
        try:
            from agents.multi_model import ModelRouter
            mr = ModelRouter()
            resp = await mr.chat(messages=[{"role":"user","content":f"将以下内容翻译成{lang_names.get(target_lang,target_lang)},保持格式:\n{text}"}], mode="fast")
            return resp.get("content", "")
        except:
            return f"[翻译失败] {text}"
