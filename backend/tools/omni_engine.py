...
class PredictEngine:
    ...
    def predict(cls, metric: str, minutes_ahead: int = 30) -> dict:
        ...
        if len(data) < 10:
            return {"predictable": False, "reason": ''}
        recent = data[-20:]
        values = [v for _, v in recent]
        avg = sum(values) / len(values)
        trend = (values[-1] - values[0]) / max(len(values), 1)
        predicted = values[-1] + trend * (minutes_ahead / 2)  # 2
...
class SelfHealing:
    ...
    def record_failure(cls, endpoint: str):
        ...
        cls._failure_count[f"total_{int(time.time()/300)}"] += 1  # 5
...
class BusinessEngine:
    ...
    def analyze_sales(cls, orders: list) -> dict:
        ''",''"
        if not orders: return {"top_products": [], "trending": [], "suggestion": ''}
        ...
        suggestion = ": " + ", ".join([p["product_id"] for p in top[:3]])
