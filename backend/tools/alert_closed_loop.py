...
AUTO_FIX_RULES = {
    ...
    "service_down": {
        "pattern": ["down", "stopped", "unhealthy", "??", "???"],
    ...
    "db_connection": {
        "pattern": ["database", "mysql", "connection", "???", "??"],
...
class AlertClosedLoop:
    ...
    async def detect_and_fix(cls, alert_title, alert_detail="", auto_fix=True):
        ...
        if auto_fix:
            ...
            for rule_name, rule in AUTO_FIX_RULES.items():
                if any(p.lower() in combined for p in rule["pattern"]):
                    ...
                    result["steps"].append("????: " + rule_name)
                    
                    for attempt in range(rule["max_retries"]):
                        result["fix_attempted"] = True
                        result["steps"].append("???? {}/{}".format(attempt+1, rule["max_retries"]))
                        
                        try:
                            proc = subprocess.run(
                                rule["fix"], shell=True, capture_output=True, text=True, timeout=30
                            )
                            result["steps"].append("????: " + rule["fix"][:80] + "...")
                            result["steps"].append("??: " + (proc.stdout.strip() or proc.stderr.strip())[:200])
                        except Exception as e:
                            result["steps"].append("??????: " + str(e)[:100])
                            break
                        ...
                        try:
                            ...
                            result["steps"].append("????: " + result["verification"][:100])
        ...
        state._save()
        logger.info("????: {} -> fix={}".format(alert_title, "OK" if result["fix_success"] else "FAIL"))
    ...
    async def run_health_check(cls):
        ...
        try:
            ...
            if disk_use and int(disk_use) > 85:
                checks.append({"type": "disk_full", "status": "fail", "detail": "????? {}%".format(disk_use)})
        except:
            pass
        ...
        try:
            ...
            if mem_use and int(mem_use) > 90:
                checks.append({"type": "memory_high", "status": "fail", "detail": "????? {}%".format(mem_use)})
        except:
            pass
        ...
        try:
            ...
            if running and int(running) < 2:
                checks.append({"type": "service_down", "status": "fail", "detail": "? {} ?????".format(running)})
        except:
            pass
