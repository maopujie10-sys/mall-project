...
async def create_alert(req: AlertCreateRequest, _=Depends(verify_token)):
    await handle_risk("L1", f" [{req.level}]", req.title)
    if req.level not in ALERT_LEVELS:
        return {"error": f": {req.level},: {list(ALERT_LEVELS.keys())}"}
    ...
    if req.level == "P1":
        ...
        state.add_emergency("human_control", f"P1: {req.title}")
