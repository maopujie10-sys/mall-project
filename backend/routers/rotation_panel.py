...
DEFAULT_DOMAINS = [
    {"domain": "tiktook.eu.cc", "active": True, "health": "ok", "type": '', "weight": 5},
    {"domain": "chxhx.eu.cc", "active": True, "health": "ok", "type": '', "weight": 5},
    {"domain": "drrgr.eu.cc", "active": True, "health": "ok", "type": '', "weight": 5},
    {"domain": "drrimrf.eu.cc", "active": True, "health": "ok", "type": '', "weight": 5},
    {"domain": "drriiu.eu.cc", "active": True, "health": "ok", "type": '', "weight": 5},
    {"domain": "duomi.eu.cc", "active": True, "health": "ok", "type": '', "weight": 4},
    {"domain": "dengruihan.eu.cc", "active": True, "health": "ok", "type": '', "weight": 4},
    {"domain": "yyawzx.eu.cc", "active": True, "health": "ok", "type": '', "weight": 3},
    {"domain": "gamed.eu.cc", "active": True, "health": "ok", "type": '', "weight": 3},
]
...
async def _check_all():
    ''"(),''"
    logger.info(f"[Rotation]  {datetime.now().strftime('%H:%M:%S')}")
    ...
    if rotated:
        logger.info(f"[Rotation]  -> {rotated}")
    if changed or rotated:
        state._save()
    logger.info(f"[Rotation] : {sum(1 for d in domains if d.get('health')=='ok')}/{len(domains)} ")
...
async def _auto_rotate() -> str:
    ''":''"
    domains = _get_domains()
    primary = next((d for d in domains if d.get("type") == ''), None)
    #  -> 
    if primary and primary.get("health") == "ok" and primary.get("active"):
        return ''
    #  -> 
    healthy = [d for d in domains if d.get("active") and d.get("health") == "ok" and d.get("type") != '']
    if not healthy:
        return ''
    # ,
...
async def get_domains(_=Depends(verify_token)):
    ...
    for d in domains:
        entry = dict(d)
        #  _check_all (last_check/latency/ip/status_code/ssl_*)
        if d.get("last_check"):
...
async def toggle_domain(req: ToggleRequest, _=Depends(verify_token)):
    ''"/''"
    await handle_risk("L2", f": {req.domain} {'' if req.active else ''}")
...
async def check_domain(req: CheckRequest, _=Depends(verify_token)):
    ''"( HTTP )''"
    await handle_risk("L1", f'', req.domain)
    for d in _get_domains():
        if d["domain"] == req.domain:
            check = await _check_one(d)
            ssl_info = await _check_ssl(d["domain"])
...
async def rotation_report(_=Depends(verify_token)):
    ...
    return {
        ...
        "primary_domain": primary["domain"] if primary else '',
        ...
        "current_rotation": primary["domain"] if (primary and primary.get("health") == "ok" and primary.get("active")) else
            (next((d["domain"] for d in active if d.get("health") == "ok"), '')),
...
async def set_weight(req: WeightRequest, _=Depends(verify_token)):
    ''"()''"
    await handle_risk("L2", '', f"{req.domain}={req.weight}")
    for d in _get_domains():
        if d["domain"] == req.domain:
...
async def manual_rotate(_=Depends(verify_token)):
    ...
    rotated = await _auto_rotate()
    state._save()
    return {"ok": bool(rotated), "rotated_to": rotated or ''}
...
async def toggle_rotation_group(group_id: str, _=Depends(verify_token)):
    ''''''
    await handle_risk("L2", f" {group_id}")
    config = _get_config()
...
async def set_rotation_weight(group_id: str, weight: int, _=Depends(verify_token)):
    ''''''
    await handle_risk("L2", f" {group_id}={weight}")
    config = _get_config()
    for r in config["rotation"]:
        if r["id"] == group_id:
...
async def add_subdomain(group_id: str, req: SubdomainRequest, _=Depends(verify_token)):
    ''''''
    await handle_risk("L2", f" {req.host}")
    config = _get_config()
    for r in config["rotation"]:
        if r["id"] == group_id:
