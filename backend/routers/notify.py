锘?""娑堟伅閫氱煡 鈥?Telegram / SMTP 閭欢 / 閽夐拤 / 浼佷笟寰俊"""
...
async def send_telegram(message: str, level: str) -> dict:
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
        return {"sent": False, "error": "Telegram 鏈厤缃?}
...
async def send_email(message: str, level: str) -> dict:
    if not SMTP_HOST or not SMTP_TO:
        return {"sent": False, "error": "閭欢鏈厤缃?}
    import smtplib
...
async def send_dingtalk(message: str, level: str) -> dict:
    if not DINGTALK_WEBHOOK:
        return {"sent": False, "error": "閽夐拤鏈厤缃?}
    try:
        async with httpx.AsyncClient(timeout=10) as c:
...
async def send_wecom(message: str, level: str) -> dict:
    if not WECOM_WEBHOOK:
        return {"sent": False, "error": "浼佷笟寰俊鏈厤缃?}
    try:
        async with httpx.AsyncClient(timeout=10) as c:
...
async def send_notification(req: NotifyRequest, _=Depends(verify_token)):
    """娑堟伅閫氱煡 鈥?Telegram / SMTP 閭欢 / 閽夐拤 / 浼佷笟寰俊"""
    await handle_risk("L1", f"锟斤拷锟斤拷通知 ({req.channel})", req.message[:50])
    ...
    if not func:
        return {"sent": False, "error": f"锟斤拷支锟街碉拷通知锟斤拷锟斤拷: {req.channel}锟斤拷锟斤拷选: {list(CHANNEL_MAP.keys()) + ['all']}"}

    return await func(req.message, req.level)

@router.get("/config")
async def notify_config(_=Depends(verify_token)):
    await handle_risk("L1", "锟介看通知锟斤拷锟斤拷")
    return {
