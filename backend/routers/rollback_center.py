...
async def create_backup(req: CreateBackupRequest, _=Depends(verify_token)):
    ''":/Nginx/''"
    await handle_risk("L2", f": {req.name} ({req.target})")
    ...
    if req.target == "database" and MALL_DB_HOST and MALL_DB_USER:
        ...
        try:
            ...
            try:
                ...
                result["error"] = f",mysqldump: {str(e2)}"
...
async def verify_backup(backup_id: str, _=Depends(verify_token)):
    ''''''
    await handle_risk("L2", '', backup_id)
    records = _load_backups()
    for r in records:
        if r["id"] == backup_id:
            path = r.get("path", '')
            if not path or not os.path.exists(path):
                ...
                r["verify_error"] = ''
                _save_backups(records)
                return {"backup_id": backup_id, "verified": False, "error": ''}
...
async def rollback(backup_id: str, _=Depends(verify_token)):
    ...
    for r in records:
        if r["id"] == backup_id:
            ...
            risk = await handle_risk("L3", f": {r['name']}", f": {target}, : {path}")
            if not risk["allowed"]:
                return risk

            if not path or not os.path.exists(path):
                return {"backup_id": backup_id, "status": "failed", "error": ''}
