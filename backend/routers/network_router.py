...
async def network_dns(domain: str, _=Depends(verify_token)):
    ''"DNS ''"
    await handle_risk("L1", f"DNS: {domain}")
    try:
        ...
        return {"ok": False, "domain": domain, "error": f": {e}"}
    except Exception as e:
        return {"ok": False, "error": str(e)}
...
async def network_http_check(url: str, _=Depends(verify_token)):
    ''"HTTP ''"
    await handle_risk("L1", f"HTTP: {url}")
    import httpx
    try:
