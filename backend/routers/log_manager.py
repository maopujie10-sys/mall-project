...
LOG_SOURCES = {
    "docker-ai": {"name": "Docker - AI", "type": "docker", "container": "ai-backend"},
    "docker-frontend": {"name": "Docker - AI", "type": "docker", "container": "ai-frontend"},
    "docker-mysql": {"name": "Docker - MySQL", "type": "docker", "container": "mysql56"},
    "docker-redis": {"name": "Docker - Redis", "type": "docker", "container": "redis"},
    "nginx-access": {"name": "Nginx - ", "type": "file", "path": "/usr/local/nginx/logs/access.log"},
    "nginx-error": {"name": "Nginx - ", "type": "file", "path": "/usr/local/nginx/logs/error.log"},
    "tomcat": {"name": "Tomcat - ", "type": "file", "path": "/opt/tomcat8/logs/catalina.out"},
    "app": {"name": "AI", "type": "file", "path": "/home/data/projects/mall/mall-project/mall-app/logs/app.log"},
}
...
async def view_logs(
    ...
    ''''''
    await handle_risk("L1", f": {source}")
    
    if source not in LOG_SOURCES:
        return {"ok": False, "error": f": {source}"}
    ...
    if info["type"] == "docker":
        try:
            ...
            return {"ok": False, "error": f"Docker: {str(e)[:100]}"}
    
    elif info["type"] == "file":
        path = info["path"]
        if not os.path.exists(path):
            return {"ok": False, "error": f": {path}"}
        try:
            with open(path, "r", encoding="utf-8", errors="ignore") as f:
                all_lines = f.readlines()
                raw_lines = [l.rstrip() for l in all_lines[-lines:]]
        except Exception as e:
            return {"ok": False, "error": f": {str(e)[:100]}"}
