...
class ModelRouter:
    ...
    def route(mode: ModelMode, preferred: str = None) -> ModelConfig:
        ...
        for c in candidates:
            ...
            # 鏈湴妯″瀷:鏃犻渶 API Key,鐩存帴鍙敤
            if cfg.is_local:
                return cfg
            # 浜戠妯″瀷:闇€瑕侀厤缃?API Key
    ...
    async def chat(model_id: str = None, messages: list = None, **kwargs) -> dict:
        """缁熶竴鑱婂ぉ鎺ュ彛 -- 鏀寔 Ollama(OpenAI鍏煎) 鍜屼簯绔?API.model_id涓篘one鏃惰嚜鍔ㄩ€夋渶浣虫ā鍨?""
