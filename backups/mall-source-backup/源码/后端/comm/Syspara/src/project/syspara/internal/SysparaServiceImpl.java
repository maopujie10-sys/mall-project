...
//		return page;
	...
	@Override
	public List<Map<String, Object>> footercontent(String lang) {
		StringBuffer queryString = new StringBuffer("SELECT * FROM T_FOOTER_HREF_CLASS WHERE LANG = '"+ lang +"'");
		Map<String, Object> parameters = new HashMap();
		Page page = this.pagedQueryDao.pagedQuerySQL(1, 99999, queryString.toString(), parameters);
		List<Map<String,Object>> elements = page.getElements();
		List<Map<String,Object>> resultaaa = new ArrayList<>();
		for (int i = 0; i < elements.size(); i++) {
			StringBuffer list = new StringBuffer("SELECT title,href FROM T_MALL_FOOTER_HREF WHERE 1 = 1 ");
			queryString.append("AND CLASS_ID=:CLASS_ID ");
			parameters.put("CLASS_ID", elements.get(i).get("CLASS_NAME"));
			queryString.append("ORDER BY SORT desc");
			List elements1 = this.pagedQueryDao.pagedQuerySQL(1, 99999, list.toString(), parameters).getElements();
			HashMap<String, Object> objectObjectHashMap = new HashMap<>();
			objectObjectHashMap.put("contens", elements1);
			objectObjectHashMap.put("categoryName", elements.get(i).get("CLASS_NAME"));
			resultaaa.add(objectObjectHashMap);
		}
		return resultaaa;
	}
