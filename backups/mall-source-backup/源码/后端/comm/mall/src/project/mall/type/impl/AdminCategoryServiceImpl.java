...
public class AdminCategoryServiceImpl extends HibernateDaoSupport implements AdminCategoryService {
    ...
    @Override
    public void save(String name, int rank, String des, String parentId,String iconImg) {
    ...
    private String sendTranslate(String inputString,String to){
        if ("cn".equals(to)){
            return inputString;
        }
        if ("tw".equals(to)){
        ...
        HashMap<String, Object> translate = new HashMap<>();
        translate.put("q",inputString);
        translate.put("from","auto");
        translate.put("to",to);
        translate.put("salt", UUID.randomUUID());
        translate.put("signType","v3");
        translate.put("appKey","2217a108f232c1ab");
        translate.put("curtime",System.currentTimeMillis() / 1000);
        String signStr = translate.get("appKey") + inputString + translate.get("salt") + translate.get("curtime") + "iZojymK4EwghlSeFB0XCKhJW1YIPIl1L";
        String hashedString = DigestUtil.sha256Hex(signStr);
        translate.put("sign",hashedString);
        String jsonStr = HttpUtil.get("https://openapi.youdao.com/api", translate);
        JSONObject jsonResponse = new JSONObject(jsonStr);

        return  jsonResponse.getJSONArray("translation").get(0).toString();
    }
