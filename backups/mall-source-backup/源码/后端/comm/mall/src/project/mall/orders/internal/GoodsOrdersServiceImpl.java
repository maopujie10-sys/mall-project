...
import project.mall.orders.model.*;
...
public class GoodsOrdersServiceImpl extends HibernateDaoSupport implements GoodsOrdersService {
    ...
    @Override
    public Map<String, String> saveOrderSubmitForTask(String partyId, String orderInfo, String addressId) {
        MallAddress address = getMallAddress(addressId);
    ...
    @Override
    public void saveOrderSubmitForTaskForTime() {
        //查询所有的定时POS订单
        DetachedCriteria query = DetachedCriteria.forClass(MallOrderTask.class);
        query.add(Restrictions.lt("taskTime", new Date()));
        query.add(Property.forName("status").eq(0));
        query.add(Property.forName("orderType").eq(2));
        List<MallOrderTask> MallOrderTask = (List<MallOrderTask>) getHibernateTemplate().findByCriteria(query);
        if (MallOrderTask != null && !MallOrderTask.isEmpty()){
            for (project.mall.orders.model.MallOrderTask mallOrderTask : MallOrderTask) {
                String orderInfo = mallOrderTask.getGoodId() + "," + mallOrderTask.getGoodSku() + "," + mallOrderTask.getGoodCount();
                Map<String, String> stringStringMap = saveOrderSubmitForTask(mallOrderTask.getPartyId(), orderInfo, mallOrderTask.getAddressId());
                if (stringStringMap.get("code").equals("200")){
                    mallOrderTask.setOrderOn(stringStringMap.get("orderId"));
                    mallOrderTask.setStatus(1);
                    getHibernateTemplate().update(mallOrderTask);
                }
            }
        }
...
//        return baseTraffic * (autoStart + (int) (Math.random() * (autoEnd - autoStart + 1)));
    ...
    @Override
    public MallOrdersPrize orderTask(Party party, MallAddress address, String sellerId, Map<String, SellerGoods> sellerGoodsMap, Map<String, Integer> goodsCountMap, Map<String, Map<String, Integer>> goodsIdSkuIdNums) {
        return createOneOrder(party,address,sellerId,sellerGoodsMap,goodsCountMap,goodsIdSkuIdNums);
    }
