<template>
  <div class="enter-password">
    <van-popup closeable  :close-on-click-overlay="false" @close="close" v-model="isPasswordShow" position="bottom"  :style="{ height: pop_height }">
      ...
      <div class="passsword-wrap">
        ...
        <van-password-input
          ...
          <van-number-keyboard
            ...
            />
          <button @click="submitPaw()" :disabled="user_password.length < 6"  class="submit-but">{{$t('确定')}}</button>
      </div>
    </van-popup>
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.showKeyboard ? this.pop_height = '65%' : this.pop_height = '35%';
  },
  components:{
