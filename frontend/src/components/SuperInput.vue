<template>
  <div class="super-input-bar" @dragover.prevent="isDragging = true" @dragleave.prevent="isDragging = false" @drop.prevent="onDrop">
    ...
    <div v-if="files.length" class="file-pills">
      <span v-for="(f, i) in files" :key="i" class="file-pill">{{ f.type === '"'image'"' ? '"'图片'"' : f.type === '"'video'"' ? '"'视频'"' : '"'文件'"' }} {{ f.name }} <button class="pill-remove" @click="files.splice(i, 1)">×</button></span>
    </div>
    <Transition name="fade"><div v-if="isDragging" class="drop-hint"><div class="drop-icon">📁</div><div>拖放文件到这里</div></div></Transition>
    ...
    <div class="input-row">
      <span class="input-prefix">{{ currentMode === '"'cmd'"' ? '"'>'"' : currentMode === '"'voice'"' ? '"'🎤'"' : currentMode === '"'camera'"' ? '"'📷'"' : '"''"' }}</span>
      ...
      <button class="action-btn" @click="fileInput.click()" title="上传文件">📎</button>
...
.input-prefix { font-size: 16px; color: #667eea; font-weight: 600; flex-shrink: 0; width: 24px; text-align: center }
