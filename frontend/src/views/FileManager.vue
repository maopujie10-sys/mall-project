...
async function doDeleteArchive(id) {
  try {
...
async function doCreateArchive() {
  ...
  try {
    await createArchive({ targets: archiveTargets.value, note: archiveNote.value, push_github: archivePushGithub.value })
...
async function loadArchives() {
  try {
    ...
    archives.value = r?.list || r?.archives || []
  } catch {}
  try {
    const r2 = await getArchiveStorage()
    archiveStorage.value = r2?.storage || []
  } catch {}
  try {
    const r3 = await listArchiveTargets()
    archiveTargetList.value = r3?.targets || r3?.list || []
  } catch {}
}

onMounted(function() { loadFiles(); loadArchives() })
</script>
