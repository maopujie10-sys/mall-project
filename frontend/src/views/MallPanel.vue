  ...
  <div class="mall-panel">
    <el-tabs v-model="activeTab" type="border-card" class="mall-tabs">
      ...
      <el-tab-pane name="dashboard">
        <template #label><el-icon><DataAnalysis /></el-icon> 仪表盘</template>
        ...
        <el-card shadow="never" style="margin-top:16px">
          <div class="card-tt">热销商品 Top10</div>
      ...
      <el-tab-pane name="merchants">
        <template #label><el-icon><Shop /></el-icon> 商家管理</template>
        <div class="tb-bar">
          <el-input v-model="merKw" placeholder="搜索商家" style="width:220px" clearable @clear="fetchMerchants" @keyup.enter="fetchMerchants" />
        ...
        <el-table :data="merchants" stripe size="small" height="520">
          ...
          <el-table-column label="操作" width="120">
            <template #default="{row}">
              <el-button size="small" @click="toggleMer(row)">{{ row.status===1?'禁用':'启用' }}</el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-tab-pane name="recharges">
        <template #label><el-icon><Wallet /></el-icon> 充值审核 <el-badge :value="recPending" :hidden="!recPending" /></template>
      ...
      <el-tab-pane name="banners">
        <template #label><el-icon><Picture /></el-icon> 轮播管理</template>
        <div class="tb-bar">
          ...
          <el-button type="success" @click="showBannerDlg()">新增轮播</el-button>
        </div>
      ...
      <el-tab-pane name="categories">
        ...
        <div class="tb-bar">
          ...
          <el-button type="success" @click="showCatDlg()">新增分类</el-button>
        </div>
        <el-table :data="categories" stripe size="small" height="480">
          ...
          <el-table-column label="操作" width="220">
            <template #default="{row}">
              ...
              <el-button size="small" :type="row.status===1?'warning':'success'" @click="toggleCatStatus(row)">{{ row.status===1?'禁用':'启用' }}</el-button>
      ...
      <el-tab-pane name="evaluations">
        ...
        <div class="tb-bar">
          <el-input v-model="evalKw" placeholder="搜索评价" style="width:220px" clearable @clear="fetchEvaluations" @keyup.enter="fetchEvaluations" />
          <el-select v-model="evalStatus" placeholder="状态" style="width:100px" clearable @change="fetchEvaluations">
      ...
      <el-tab-pane name="attrCats">
        ...
        <div class="tb-bar">
          ...
          <el-button type="success" @click="showAttrCatDlg()">新增分类</el-button>
        </div>
      ...
      <el-tab-pane name="attrs">
        <template #label><el-icon><SetUp /></el-icon> 属性管理</template>
        <div class="tb-bar">
          <el-input v-model="attrCatFilter" placeholder="属性分类ID" style="width:220px" clearable @clear="fetchAttrs" @keyup.enter="fetchAttrs" />
          <el-button type="primary" @click="fetchAttrs">搜索</el-button>
          <el-button type="success" @click="showAttrDlg()">新增属性</el-button>
        </div>
    ...
    <!-- 调余额弹窗 -->
    <el-dialog v-model="balDlg.show" title="调整余额" width="400px">
      <el-form label-width="80px">
        <el-form-item label="用户">{{ balDlg.user }}</el-form-item>
        <el-form-item label="金额"><el-input v-model="balDlg.amount" placeholder="正数增加,负数减少" /></el-form-item>
    ...
    <!-- 入驻审核弹窗 -->
    <el-dialog v-model="applyDlg.show" title="入驻申请审核" width="500px">
      <el-table :data="applies" stripe size="small" height="400">
        <el-table-column prop="id" label="ID" width="60" />
        <el-table-column prop="name" label="商家名" width="120" />
        <el-table-column prop="phone" label="手机号" width="130" />
    ...
    <!-- 轮播编辑弹窗 -->
    <el-dialog v-model="bannerDlg.show" :title="bannerDlg.uuid?'编辑轮播':'新增轮播'" width="500px">
      <el-form label-width="80px">
    ...
    <!-- 分类编辑弹窗 -->
    <el-dialog v-model="catDlg.show" :title="catDlg.uuid?'编辑分类':'新增分类'" width="500px">
      <el-form label-width="80px">
    ...
    <el-dialog v-model="attrDlg.show" :title="attrDlg.uuid?'编辑属性':'新增属性'" width="400px">
      <el-form label-width="100px">
        <el-form-item label="属性分类ID"><el-input v-model="attrDlg.categoryId" /></el-form-item>
...
const kpis = reactive([
  ...
  { label:'待审核', value:0, color:'c4', key:'pending_recharge' },
])
const orderStats = reactive([
  { l:'总订单', v:0 },{ l:'进行中', v:0 },{ l:'已完成', v:0 },{ l:'已取消', v:0 },
])
const topGoods = ref([])
...
async function doAdjust() {
  try {
    ...
    ElMessage.success('余额已调整'); balDlg.show=false; fetchUsers()
  } catch(e) { ElMessage.error(e.message) }
}
...
async function auditProd(row, status) {
  try {
    ...
    ElMessage.success(status===1?'已上架':'已下架'); fetchProducts()
  } catch(e) { ElMessage.error(e.message) }
}
...
async function delEval(row) {
  try {
    await ElMessageBox.confirm('确认删除该评价？','删除确认',{type:'warning'})
...
async function delAttrCat(row) {
  try {
    await ElMessageBox.confirm('确认删除该属性分类？','删除确认',{type:'warning'})
...
async function delAttr(row) {
  try {
    await ElMessageBox.confirm('确认删除该属性？','删除确认',{type:'warning'})
