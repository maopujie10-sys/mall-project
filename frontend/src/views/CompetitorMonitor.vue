<template>
  <div class="page-shell">
    ...
    <el-card><template #header>🔍 添加监控目标</template>
      ...
      <el-button type="primary" style="margin-top:8px" @click="addTarget">添加监控</el-button>
    </el-card>
    <el-card style="margin-top:16px"><template #header>📊 监控报告</template>
      <el-empty v-if="!reports.length" description="暂无监控数据"/>
      <div v-for="r in reports" :key="r.url" class="comp-item">
        <span class="comp-name">{{ r.name }}</span><el-tag size="small">{{ r.status }}</el-tag>
      </div>
    </el-card>
  </div>
</template>
...
const targetUrl = ref(""); const targetName = ref(""); const reports = ref([])
async function addTarget() {
  if (!targetUrl.value) return
  try {
    const r = await agentApi.post("/agent/collab/run", { goal: "分析竞品: " + targetName.value + " (" + targetUrl.value + ")" })
    if (r?.data?.ok) { reports.value.push({ name: targetName.value, url: targetUrl.value, status: "监控中" }); ElMessage.success("已添加"); targetUrl.value = ""; targetName.value = "" }
...
.comp-name { color: #e0e0e0; font-size: 13px; }
